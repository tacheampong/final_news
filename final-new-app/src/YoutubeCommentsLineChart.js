...
const YotubeCommentsLineChart = ({data}) => {
   ...
   var aggregated = Array.from(
     d3.rollup(
       data,
       (v) => d3.sum(v, (d) => d.avg_comment_count),
       (d) => d.topic,
       (d) => d.date,
     ),
     ([topic, dateMap]) => ({
   ...
   const yMax = d3.max(aggregated, (topic) =>
     d3.max(topic.values, (d) => d.avg_comment_count),
   );
